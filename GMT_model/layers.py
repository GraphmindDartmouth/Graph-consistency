...
class GCNConv_for_OGB(MessagePassing):
    def __init__(self, emb_dim, edge_attr_dim):
        ...
        self.edge_lin = torch.nn.Linear(edge_attr_dim, emb_dim)

    def forward(self, x, edge_index, edge_attr):
        x = self.linear(x)
        edge_embedding = self.bond_encoder(edge_attr) if edge_attr.dtype is not torch.float else self.edge_lin(edge_attr)
