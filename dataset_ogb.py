...
class PygGraphPropPredDataset(InMemoryDataset):
    def __init__(self, name, root = 'dataset', transform=None, pre_transform = None, meta_dict = None, skip_collate=False):
        ...
        self.__num_classes__ = self.num_tasks
        ...
        # self.data.x = self.data.x.to(dtype=torch.float)
        
        # if 'edge_attr' in self.data:
        #     self.data.edge_attr = self.data.edge_attr.to(dtype=torch.float)
